layui.config({
    base : "js/index/"
}).use(['jquery','layer','element','bodyTab','form'], function () {
    var $ = layui.jquery,
        layer = layui.layer,
        form = layui.form,
    ...
    if($(".userName").text().length > 6){
      $(".userName").text($(".userName").text().substring(0,6) +"...");
    }
    ...
    function lockPage(){
      layer.open({
        ...
        '<div class="admin-header-lock-name" id="lockUserName">'+vm.user.userName+'</div>' +
        '<div class="input_btn">'+
        '<input type="password" class="admin-header-lock-input layui-input" autocomplete="off" placeholder="请输入密码解锁.." name="lockPwd" id="lockPwd" />'+
        '<button class="layui-btn" id="unlock">解锁</button>'+
        '</div>'+
        '<p>请输入锁定账号的密码，否则不会解锁成功哦！！！</p>'+
        '</div>',
        closeBtn : 0,
        shade : 0.9
      });
    ...
    $("body").on("click","#unlock",function(){
      if($(this).siblings(".admin-header-lock-input").val() == ''){
        ...
        $.ajax({
          type: "POST",
          url: "sys/user/unlock",
          data: JSON.stringify({"password":$(this).siblings(".admin-header-lock-input").val()}),
          dataType: "json",
